...
#     This lets the full pipeline run and be tested before the model is trained.
#
...
# Tune after capturing real baseline data
...
def _load_model():
    ...
    try:
        ...
        log.info("[Inference] ONNX model loaded from %s", model_path)
        return sess
...
def _onnx_score(sess, snapshot: np.ndarray) -> dict:
    ...
    snapshot shape: (>=WINDOW_SIZE, N_FEATURES)
    """
