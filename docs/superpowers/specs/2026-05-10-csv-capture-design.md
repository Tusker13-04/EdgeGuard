...
The prototype focuses on vibration-dominant anomaly signatures (MPU-6050) while preserving modular support for current telemetry in the final Arduino UNO Q deployment.

## 2. Sensor Stack
| Sensor | Signal | Interface |
|---|---|---|
| MPU-6050 | accel_x, accel_y, accel_z | I2C → ESP8266 |
| 10kΩ NTC Thermistor | temp | ADC → ESP8266 |
| Current (reserved) | — | ACS712 on UNO Q only |

## 3. Capture Parameters
| Parameter | Value |
|---|---|
| Sample rate | 1000 Hz |
| Window size | 500 rows (500ms) |
...
4. Recording window opens: collects exactly `duration_seconds × 1000` rows from the live buffer.
5. Capture array sliced into non-overlapping 500-row windows.
6. Each window saved as one CSV file.

## 6. Edge Impulse CSV Format
Each file contains exactly 501 lines (1 header + 500 data rows):
```
timestamp,accX,accY,accZ,temp
0,1.023,-0.512,9.812,25.4
1,1.031,-0.498,9.801,25.4
...
499,1.019,-0.501,9.808,25.3
```
- `timestamp` is a relative integer (0–499). Edge Impulse expects relative, not wall-clock timestamps.
- One file = one 500ms training sample = one class label.
